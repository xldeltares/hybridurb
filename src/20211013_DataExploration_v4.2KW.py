...
def readTablemq_v4_1(X, filename, version, tablepath):
    ...
    # top down
    d.loc[:,'UST'] = d.loc[:,'nc_ITCTmitc_yr']; d.loc[:,'UST'] = d.loc[:,'UST'].round(1); d.loc[d.UST <= 0.0, 'UST'] = 0.01
...
def readTablemq_v4_1_2(X, keyword, version):
    # read calibration table
    # !NOTE: the calculaiton method might be wrong, need more physical meaning
    table = pd.read_csv(path + r"CalibrationEvents\\" + version +
...
def modelCalibration(equ, d, plot = True):
    ...
    logit = sm.Logit(y_train, X_train); model = logit.fit(method='bfgs')
...
def eventValidation(model, equ, d, plot = True):
    ...
    for event in total_events:
        ...
        if plot:
            ...
            plt.tight_layout()
            plt.savefig(path + "CalibrationEvents\\" + version + '\\' + event + f'_{equ}.png', dpi=300)
            y_pred.to_csv(path + "CalibrationEvents\\" + version + '\\' + event + f'_{equ}.csv')
            plt.close()
    ...
    scores.to_csv(path + "CalibrationEvents\\" + version + '\\' + 'scores_' + equ + '.csv')
    return f1s
...
tablepath_cal = "J:\\LocalWorkspace\\hybridtest\\data_preprocessing\\eventcsv_raingen_diffuse_cal\\"
tablepath_val = "J:\\LocalWorkspace\\hybridtest\\data_preprocessing\\eventcsv_raingen_val\\"
version = 'v4.1'
X = read_gpickle("J:\\LocalWorkspace\\hybridtest\\sewer\\sewer_graph_networkx_v4.1.gpickle" )
...
d_C = pd.concat([readTablemq_v4_1(X, cal_eventlist[keyword], version, tablepath_cal) for keyword in Events])
...
for v in ['NS', 'PS', 'OT','d', 'DST', 'IT']:  d.loc[:,v], lams[v] = stats.boxcox(d.loc[:,v])
# validation events
...
d_V = pd.concat([readTablemq_v4_1(X, val_eventlist[keyword], version, tablepath_val) for keyword in V_Events])
...
# save lams
pickle.dump(lams, open(path + "CalibrationEvents\\" + version +  "\\sewer_" + version  + "_Calibrated_lambda.pickle", "wb"), protocol=2)
...
nx.write_gpickle(X, path + "CalibrationEvents\\" + version +  "\\sewer_" + version + "_Calibrated.gpickle")
...
# equ op4
new_d = d[['n_F','OT', 'd']].join(cat); new_d.Cat = new_d.Cat.fillna(0)
equ = 'n_F ~ C(Cat)+OT+d';  equs.append(equ)
#aps_ = kfoldValidation(equ, new_d); aps.append(np.mean(aps_));APs.append(aps_)
model = modelCalibration(equ, new_d); model.summary()
...
# equ op6
new_d = d[['n_F', 'NS', 'OT', 'PS']].join(cat); new_d.Cat = new_d.Cat.fillna(0)
equ = 'n_F ~ C(Cat)+NS+OT+PS';  equs.append(equ)
#aps_ = kfoldValidation(equ, new_d); aps.append(np.mean(aps_));APs.append(aps_)
model = modelCalibration(equ, new_d); model.summary()
...
# equ op7
new_d = d[['n_F', 'NS', 'PS']].join(cat); new_d.Cat = new_d.Cat.fillna(0)
equ = 'n_F ~ C(Cat)+NS+PS';  equs.append(equ)
#aps_ = kfoldValidation(equ, new_d); aps.append(np.mean(aps_));APs.append(aps_)
model = modelCalibration(equ, new_d); model.summary()
...
# equ op8
new_d = d[['n_F', 'NS', 'PS', 'd']].join(cat); new_d.Cat = new_d.Cat.fillna(0)
equ = 'n_F ~ C(Cat)+NS+PS+d';  equs.append(equ)
#aps_ = kfoldValidation(equ, new_d); aps.append(np.mean(aps_));APs.append(aps_)
model = modelCalibration(equ, new_d); model.summary()
