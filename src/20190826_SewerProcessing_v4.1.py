...
while count < 10:
    ...
    if len(nx.difference(X4, X_optimised).edges()) != 0:
        ...
        break
    if count == 9:
        print(f'OPTIMISATION FINISHED (Maixmum iteration reached): number of loop {count+1}')
...
##########################################################
# all nodes
df1 = pd.DataFrame.from_dict(dict(X9.nodes(data = 'Outfall')), orient='index', columns=['Outfall'])
df2 = pd.DataFrame.from_dict(dict(X9.nodes(data = 'tc_min')), orient='index', columns=['tc_min'])
df3 = pd.DataFrame.from_dict(dict(X9.nodes(data = 'us_area_m2')), orient='index', columns=['us_area_m2'])
df = pd.concat([df1, df2, df3], axis = 1)
df.to_csv(path + "sewer\\" + "sewer_graph_networkx_v4.1.csv")
# outfall
df = pd.DataFrame(columns = ['Number of nodes', 'Effective upstream area (ha)',
'Concentraiton time (min)'])
outfalls = [x for x in X9.nodes if X9.nodes[x]['type'].lower() in ['outfall']]
XX = X9.reverse()
for o in outfalls:
    df.loc[o, :] = len(nx.dfs_tree(XX,o).nodes), round(X9.nodes[o]['us_area_m2']/10000,1), X9.nodes[o]['tc_min']
