...
class T_func:
    def __init__(self):
        ...
        self.f = interpolate.NearestNDInterpolator(np.vstack([ii.ravel(), dd.ravel()]).T, tt.ravel())
    def calc(self, data):
        ...
        if data.ndim == 1:
            ...
            data_T = np.zeros(data.shape[0],)*np.nan
...
def wetPipe(condshape, condwidth, condheight, sedimentdepth = None):
    ...
    Area = {
        ...
        'ARCH': lambda w, h, theta: w*(h-w/2.0) + 1.0/2.0*np.pi*(w/2.0)**2,
    ...
    minussediment_Area = {
        ...
        'OVAL': lambda w, h, s, theta: Area['OVAL'](w,h,theta) - arc_area(w/2., arc_beta(w/2., s)) if s <= w/2. \
                                    else (arc_area(w/2., np.pi) + (h-w/2.-s)*w  if s <= (h - w/2.) \
                                    else arc_area(w/2., arc_beta(w/2., h-s))),
        'EGG': lambda w, h, s, theta: Area['EGG'](w,h,theta) - arc_area((h-w)/2., arc_beta((h-w)/2., s)) if s <= arc_height((h-w)/2., theta*2.) \
                                    else (Area['EGG'](w,h,theta) - arc_area((h-w)/2., 2*theta) - egg_trap_area(w, h, s, theta) if s <= h - arc_height(w/2., 2.*np.pi - theta*2.) \
                                    else arc_area(w/2., arc_beta(w/2., h-s))),
        'ARCH': lambda w, h, s, theta: arc_area(w/2., np.pi) + (h-w/2.-s)*w if s <= h - w/2. \
                                    else arc_area(w/2., arc_beta(w/2., h-s)),
        'CIRC': lambda w,h, s, theta: arc_area(w/2., 2.*np.pi) - arc_area(w/2., arc_beta(w/2., s)),
        'OT': lambda w,h, s, theta: (2.0*w + 2.0*theta*h) * h/2 - (2.0*w + 2.0*theta*s) * s/2.,
        }
    minussediment_Perimeter = {
        'RECT': lambda w, h, s, theta: 2.*w + 2.*(h-s),
        'OVAL': lambda w, h, s, theta:  Perimeter['OVAL'](w,h,theta) - arc_arc(w/2., arc_beta(w/2., s)) + arc_chord(w/2., arc_beta(w/2., s)) if s <= w/2. \
                                    else (arc_arc(w/2., np.pi) + 2*(h-w/2. -s) + w  if s <= (h - w/2.) \
                                    else  arc_arc(w/2., arc_beta(w/2., h-s)) + arc_chord(w/2., arc_beta(w/2., h-s))),
        'EGG': lambda w, h, s, theta: Perimeter['EGG'](w,h,theta) - arc_arc((h-w)/2., arc_beta((h-w)/2., s)) + arc_chord((h-w)/2., arc_beta((h-w)/2., s)) if s <= arc_height((h-w)/2., theta*2.) \
                                    else (arc_arc(w/2., 2.*np.pi - 2.*theta) + egg_trap_peri(w,h,s,theta) - arc_chord(w/2., 2.*np.pi - 2.*theta) if s <= arc_height(w/2., 2.*np.pi - theta*2.) \
                                    else arc_arc(w/2., arc_beta(w/2., h-s))),
        'ARCH': lambda w, h, s, theta: arc_arc(w/2., np.pi) + 2.*(h-w/2.-s) + w if s <= h - w/2. \
                                    else arc_arc(w/2., arc_beta(w/2., h-s)) + arc_chord(w/2., arc_beta(w/2., h-s)),
        'CIRC': lambda w,h, s, theta: arc_arc(w/2., 2.*np.pi-arc_beta(w/2., s)) + arc_chord(w/2., arc_beta(w/2., s)),
        'OT': lambda w,h, s, theta: w + 2.0*s*theta + 2.0*(h-s)*(1.0 + theta**2.0)**(1.0/2.0),
        }
    arc_beta = lambda r, d: 2 * np.arccos((r - d)/r) # d is the sediment height
    arc_height = lambda r, beta: r * (1-np.cos(beta/2.))
    arc_area = lambda r, beta: 1./2. * r**2 * (beta - np.sin(beta))
    arc_arc = lambda r, beta: beta * r
    arc_chord = lambda r, beta: 2 * r * np.sin(beta/2)
    egg_trap_area = lambda w, h, s, theta: (arc_chord((h-w)/2., 2.*theta) + \
                                        egg_b(arc_chord((h-w)/2., 2.*theta),(s-arc_height((h-w)/2., theta*2.)), theta))*\
                                        (s-arc_height((h-w)/2., theta*2.))
    egg_trap_peri = lambda w, h, s, theta: (arc_chord((h-w)/2., 2.*theta) + \
                                        egg_b(arc_chord((h-w)/2., 2.*theta),(s-arc_height((h-w)/2., theta*2.)), theta) + \
                                        (s-arc_height((h-w)/2., theta*2.))/np.sin(theta)) * 2
    egg_b = lambda a, ds, theta: (a * np.tan(theta) + ds)/np.tan(theta)
    if not sedimentdepth or sedimentdepth == 0:
        A, P = (Area[condshape](condwidth, condheight, theta), 
        Perimeter[condshape](condwidth, condheight, theta))
    else:
        A, P = (minussediment_Area[condshape](condwidth, condheight,sedimentdepth, theta), 
        minussediment_Perimeter[condshape](condwidth, condheight, sedimentdepth, theta))
    return A,P
...
def V_CW(D,Sf,Ks):
    ...
    Sf_neg = False
    if Sf < 0: Sf = np.absolute(Sf); Sf_neg = True
    velocity = -2.0 * (2.0 * 9.807 * D * Sf)** (1.0/2.0) \
        * np.log10(Ks * (10.0 ** (-3.0)) / (3.7 * D) + (2.51 * 10.0**(-6.0))/(D * ( 2.0 * 9.807 * D * Sf) ** (1.0/2.0)))
    if Sf_neg: velocity = -1.0 * velocity
    return velocity
...
def breakLoop(X):
    ...
    try: 
        ...
        while loop:
            ...
            out_nodes = [nn for nn in loop_nodes if len([i for i in X.out_edges(nn)]) > 1]
            ...
            if len(loop_nodes)== 2:
                ...
                for o_n in out_nodes:
                    try:
                        l = {c_n: nx.shortest_path_length(X, source=o_n, target= c_n) for c_n in critical_nodes if c_n != o_n and nx.has_path(X, source=o_n, target= c_n)}
                        ...
                        pp = [X.nodes[o_n]['chambfloor_m'] - X.nodes[nn]['chambfloor_m'] for nn in p]
                        ...
                        point = [i for i in range(1, len(p)) if pp[i] <0 and pp[i] - pp[i-1] <= 0]
                        if len(out_nodes) == 1:
                            point = range(1, len(p))
                        try:
                        ...
                        for i in range(point):
                            ...
                            if attrs[0]['type'].lower() != 'cond':
                                print('\tSKIP: Break point is ' + attrs[0]['type'].lower() + ' for ' + us_node + '-->' + ds_node + ', go to next out_node...')
                                break
                            ...
                            print('REVERSE: Break point found: ' + us_node + '-->' + ds_node + ", go to next link...")
                    except IndexError:
...
def reverseOutfall(X, levels = None):
    ...
    for o in outfalls:
        try:
            if levels[o] and X.nodes[o]['chambfloor_m'] < levels[o]: #NEW! only reverse if the chambfloor is lower than water level
        ...
        if len(X.in_edges(o)) < 1 and len(X.out_edges(o)) > 0:
            for e in list(X.out_edges(o)):
                us_node,ds_node = e
                attrs = X[us_node][ds_node]
                if attrs[0]['type'] == 'cond' and X.nodes[us_node]['chambfloor_m'] <= X.nodes[ds_node]['chambfloor_m']:
                    X.add_edges_from([(ds_node, us_node, attrs[0])])
                    X.remove_edge(us_node, ds_node)
                    print('REVERSE: ' + us_node + '-->' + ds_node + " (gradient error), go to next link...")
        try:
            ...
            for edge in edges:
                ...
                if  edge[2]['type'] == 'cond' and X.nodes[o]['chambfloor_m'] > edge[2]['ds_invert_m']:
                    X[edge[0]][edge[1]][0].update({'ds_invert_m':X.nodes[o]['chambfloor_m']})
    ...
    for o1 in outfalls:
        ...
        for o2 in outfalls:
            ...
            if len(shared) == 0:
                ...
                s = [n for n in X.subgraph(shared) if X.subgraph(shared).out_degree(n) == 0][0]
                # path length
                l1 = [nx.dijkstra_path_length(X, source = s, target = o1, weight = weight_var) for s in shared]
                l2 = [nx.dijkstra_path_length(X, source = s, target = o2, weight = weight_var) for s in shared]
...
def recoverLeaf(X2, X1):
    ...
    def reverseEdgeDict(d):
        d_new = d.copy()
        d_new.update({'gradient_mdm': (- d['gradient_mdm'] if d['gradient_mdm'] else 0), 
    ...
    for c in nx.connected_components(nx.Graph(G)):
        ...
        ss = [n for n in SG.nodes if len(SG.out_edges(n)) == 0 and SG.degree(n) != X.degree(n)]
        if len(ss)>=1:
            stem = ss[0]
            depr = [n for n in SG.nodes if len(SG.out_edges(n)) == 0 and SG.degree(n) == X.degree(n) and n != stem]
        else:
            ...
            for me in mismatch_es:
                ...
                if me not in X_new.edges():
                    ...
                    if np.any([nx.has_path(X_new, ns, o) for o in outfalls]):
                        ...
                        # fix depressions
                        for nd in depr:
                            nds_path = nx.shortest_path(nx.Graph(SG), source=nd, target=ns)
                            for e in zip(nds_path[:-1], nds_path[1:]):
                                if e not in X_new.edges() and (e[1], e[0]) in X_new.edges():
                                    # the defination of us ds is the corrected edge direction, not the original
                                    us,ds = e; d = dict(X_new[ds][us])[0]
                                    d = reverseEdgeDict(d)
                                    X_new.add_edges_from([(us,ds,d)])
                                    X_new.remove_edge(ds,us)
                                    print('\t' + nd + '(depressions) is sucessfully corrected!')                       
                    else:
                        raise(ValueError(ns + ' can still not connect to path!'))
    # chekc if all nodes in gone nodes now connects to an outfall
    if np.all([np.any([nx.has_path(X_new, ns, o) for o in outfalls]) for n in gone_nodes]):
            print('ALL NODES FOUND OUTFALL! After recovered components: ' + str(nx.number_connected_components(nx.Graph(X_new))) + '\n')
    return X_new
    ...
    for n in split_nodes:
        ...
        if len(outs_dict) > 0:
            for oo in outs_dict.keys():
                l = nx.dijkstra_path_length(X,n,oo, 
            ...
            opath = nx.dijkstra_path(X,n,o, 
                    weight = lambda u, v, e: np.abs(X.nodes[v]['chambfloor_m'] - X.nodes[u]['chambfloor_m'] )
                    )
        else:
            ...
            for me in mismatch_es:
                ...
                if me not in X_new.edges():
                    ...
                    if np.any([nx.has_path(X_new, ns, o) for o in outfalls]):
                        ...
                        raise(ValueError(ns + ' can still not connect to path!'))
    # chekc if all nodes in gone nodes now connects to an outfall
    if np.all([np.any([nx.has_path(X_new, n, o) for o in outfalls]) for n in gone_nodes]):
            print('ALL NODES FOUND OUTFALL! After recovered components: ' + str(nx.number_connected_components(nx.Graph(X_new))) + '\n')
    ...
    for e in X.edges(data = True):
        ...
        if e[2]['shape'] != 0 and e[2]['gradient_mdm'] != 0 and e[2]['shape'] != None:
            ...
            if Dh!= 0 and D != 0:
                ...
                Qmax = A * vmax
                if Qmax != 0:
                    Hfmax = Hf_DW(f_DW(Re_R(np.abs(vmax), Dh), Ks, Dh), np.abs(vmax), L, Dh)
                else:
                    Hfmax = 0.
            else:
                Qmax = Qcap
        ...
        e[2]['Hfmax_m'] = Hfmax
        e[2]['Qmin_m3ds'] = Qmin 
        Zloss1 = (e[2]['us_invert_m'] - X.nodes[us]['chambfloor_m']\
                if e[2]['us_invert_m'] - X.nodes[us]['chambfloor_m'] > 0 else 0.0)
        Zloss2 = (-(e[2]['us_invert_m'] - e[2]['ds_invert_m'])\
                if (e[2]['us_invert_m'] - e[2]['ds_invert_m']) < 0 else 0.0)
        Zloss3 = ((e[2]['us_invert_m'] - e[2]['ds_invert_m'])\
                if (e[2]['us_invert_m'] - e[2]['ds_invert_m']) > 0 else 0.0)
        e[2]['Zloss1_m'] =  Zloss1
        e[2]['Zloss2_m'] =  Zloss2
        e[2]['Zloss3_m'] =  Zloss3
...
def multi2uniDiGraph(X):
    ...
    for us,ds in X.edges():
        ...
        if len(Xdata) > 1:
            new_weight = {k: np.sum([np.array(Xdata[e][k], dtype = np.float) for e in Xdata]) \
                if type(Xdata[0][k]) != type('str') \
                else Xdata[0][k] for k in Xdata[0].keys()}
...
def calcDAGTc(X, method = 'CW_cap'):
    ...
    for n in X.nodes:
        ...
        if method == 'CW_cap':
            ...
            t_path = nx.single_source_dijkstra_path_length(sub_network,n, \
                weight = lambda u, v, e: e['length_m']/(e['vcap_mds']) if e['vcap_mds'] > 0 else 0)
            tc = max([t_path[k] if not np.isnan(t_path[k]) or t_path[k] == 0  else np.nan for k in t_path ])
            X.nodes[n]['tc_min'] = (int(tc/60.0/5.0)+1)*5 if not np.isnan(tc) else np.nan # longest time of travel path in minutes (rounded to 5min)
        elif method == 'CW_max':
            # calculate trvel time using colebrook-white under capacity condition
            t_path = nx.single_source_dijkstra_path_length(sub_network,n, \
                weight = lambda u, v, e: e['length_m']/(e['vmax_mds']) if e['vmax_mds'] > 0 else 0)
            tc = max([t_path[k] if not np.isnan(t_path[k]) else 0 for k in t_path ])
            X.nodes[n]['tc_min'] = (int(tc/60.0/5.0)+1)*5 # longest time of travel path in minutes (rounded to 5min)
        elif method == 'C':
            # calculate longest flow path length and slope for cater lag equation
            l_path = nx.single_source_dijkstra_path_length(sub_network,n, weight = 'length_m')
            (ns,l) = max(l_path.items(), key=itemgetter(1))
            l_m = l #maximum flow path length meters
            s_m = X.nodes[ns]['chambroof_m'] - X.nodes[n]['chambfloor_m'] #maximum elevation differences meters
            tc_min = (100 * (l_m/1609.344)**0.6 * ((s_m*3.281)/(l_m/1609.344))**(-0.3) if l_m != 0 and s_m  > 0 else 0)
            ...
            try:
                ...
                tc_min = (0.3 * (l_m**0.76 / (s_m/l_m)**0.19))/(1+3*np.sqrt(miu * (2-miu))) 
                X.nodes[n]['tc_min'] = (int(tc_min/5.0)+1)*5 # (rounded to 5min)
            except:
                X.nodes[n]['tc_min'] = np.nan 
                print(n + ': no tc_min calculated')
        elif method == 'meanv':
            mean_v = np.nanmean([e[2] for e in sub_network.edges(data = 'vcap_mds') if e[2]>0])
            if mean_v != 0 and ~np.isnan(mean_v):
                # calculate trvel time using colebrook-white under capacity condition
                l_path = nx.single_source_dijkstra_path_length(sub_network,n, \
                    weight = lambda u, v, e: e['length_m'])
                l =np.nanmean(np.array([l_path[k]  for k in l_path ])    )
                tc = l/mean_v
            else: 
                tc = np.nan
            X.nodes[n]['tc_min'] = (int(tc/60.0/5.0)+1)*5 if not np.isnan(tc) else np.nan 
        else:
            print('method unknown, please choose between: "CW_cap": Colebrook-while under capacity condition,',
...
def calcDAGStaticFI(X):
    ...
    for e in X.edges(data = True):
        us,ds = e[:2]
        if e[2]['Dh_m']:
            vmax =  V_CW(e[2]['Dh_m'], X.nodes[us]['hydraulicgradient_o'], e[2]['rcw_mm'])
            Qmax = e[2]['A_m2'] * vmax
            e[2]['Qo_m3ds'] = Qmax
            X.nodes[us]['Qo_m3ds'] = e[2]['Qo_m3ds']
            vmin =  V_CW(e[2]['Dh_m'], X.nodes[us]['hydraulicgradient_o_0'], e[2]['rcw_mm'])
            Qmin = e[2]['A_m2'] * vmin
            e[2]['Qo_0_m3ds'] = Qmin
...
def calcConcentrationTime(X, method = 'CW_cap'):
    ...
    for n in X.nodes:
        ...
        if method == 'CW_cap':
            # calculate trvel time using colebrook-white under capacity condition
            t_path = nx.single_source_dijkstra_path_length(sub_network,n, \
                weight = lambda u, v, e: e[0]['length_m']/(e[0]['vcap_mds']) if e[0]['vcap_mds'] > 0 else 0)
            tc = max([t_path[k] if not np.isnan(t_path[k]) else 0 for k in t_path ])
            X.nodes[n]['tc_min'] = (int(tc/60.0/5.0)+1)*5 # longest time of travel path in minutes (rounded to 5min)
        elif method == 'CW_max':
            # calculate trvel time using colebrook-white under capacity condition
            t_path = nx.single_source_dijkstra_path_length(sub_network,n, \
                weight = lambda u, v, e: e[0]['length_m']/(e[0]['vmax_mds']) if e[0]['vmax_mds'] > 0 else 0)
            tc = max([t_path[k] if not np.isnan(t_path[k]) else 0 for k in t_path ])
            X.nodes[n]['tc_min'] = (int(tc/60.0/5.0)+1)*5 # longest time of travel path in minutes (rounded to 5min)
        elif method == 'C':
            # calculate longest flow path length and slope for cater lag equation
...
def calcStaticFI_path(X):
    ...
    for e in X.edges(data = True):
        ...
        Zloss1 = (edge_data['us_invert_m'] - X.nodes[us]['chambfloor_m']\
                if edge_data['us_invert_m'] - X.nodes[us]['chambfloor_m'] > 0 else 0.0)
        Zloss2 = (-(edge_data['us_invert_m'] - edge_data['ds_invert_m'])\
                if (edge_data['us_invert_m'] - edge_data['ds_invert_m']) < 0 else 0.0)
        e[2]['Zloss1_m'] =  Zloss1
        e[2]['Zloss2_m'] =  Zloss2
    ...
    for o in outfalls:
        ...
        ons_l = nx.single_source_dijkstra_path_length(XX.subgraph(nx.dfs_tree(XX,o).nodes), o,\
                weight = 'length_m')
        ons_zloss1 = nx.single_source_dijkstra_path_length(XX.subgraph(nx.dfs_tree(XX,o).nodes), o, \
                weight = 'Zloss1_m')
        ons_zloss2 = nx.single_source_dijkstra_path_length(XX.subgraph(nx.dfs_tree(XX,o).nodes), o, \
                weight = 'Zloss2_m')
        ons_zloss = nx.single_source_dijkstra_path_length(XX.subgraph(nx.dfs_tree(XX,o).nodes), o, \
                weight = lambda u, v, e: e[0]['Zloss1_m'] + e[0]['Zloss2_m'])
        ons_Hf_m = nx.single_source_dijkstra_path_length(XX.subgraph(nx.dfs_tree(XX,o).nodes), o, \
                weight = 'Hf_m')  
        for n in sub_network.nodes:
            Path_dZ_m[n] = sub_network.nodes[n]['chambfloor_m'] - sub_network.nodes[o]['chambfloor_m']
        Path_L_m.update(ons_l)
        ons_l_mean = float(sum(ons_l.values())) / len(ons_l)
        Path_rl_m.update({k: ons_l[k]/ons_l_mean if ons_l_mean != 0 else 0 for k in ons_l})
        Path_Slope_mdm.update({k: Path_dZ_m[k]/ons_l[k] if ons_l[k] != 0 else 0 for k in ons_l})
    ...
    for n in X.nodes:
        X.nodes[n].update({
                        'Path_L_m': Path_L_m[n], 'Path_rl_m': Path_rl_m[n], \
                        'Path_dZ_m': Path_dZ_m[n], 'Path_Slope_mdm': Path_Slope_mdm[n], \
                        'Path_Zloss1_m':Path_Zloss1_m[n], 'Path_Zloss2_m': Path_Zloss2_m[n], 'Path_Zloss_m': Path_Zloss_m[n], \
...
def linearReservior(R_df, A):
    ...
    for i in range(1, len(R)):
        Q[i] = Q[i-1]*np.exp(-A*5.) + R[i-1]*(1-np.exp(-A*5.))
    return pd.Series(Q/3600., index = R_df.index)
...
def calcQ(X, sim_n_RR):
    ...
    sim_n_RO = pd.DataFrame({n:linearReservior(sim_n_RR[n]/(10**3) * X.nodes[n]['cont_area_m2'], 15.) \
                            if n in sim_n_RR.columns and X.nodes[n]['type'].lower() not in ['outfall'] \
                            else sim_n_RR.iloc[:,0]*0. for n in X.nodes })
    ...
    for n in XX.nodes:
        ...
        RO_t = pd.concat([sim_n_RO[[nsi for nsi in ns if nsi in sim_n_RO.columns]]], axis=1).sum(axis=1) 
        # subcatchment discharge - accumulation in time (convolve)
        if X.nodes[n]['tc_min']:
            window = int(X.nodes[n]['tc_min']/5)
            if window > 1:
                Q_tc = np.convolve(RO_t, np.ones(window,)/window, mode='full')[:RO_t.size]   
            else:
                Q_tc = RO_t
            sim_nc_RO[n] = RO_t
            sim_nc_Q[n] = Q_tc
...
def calcOT(X, sim_n_RR):
    ...
    for o in outfalls:
        ...
        RR_t = np.mean(sim_n_RR[[nsi for nsi in ns if nsi in sim_n_RR.columns]].values, axis = 1)
        if not np.any(np.isnan(RR_t)):
            ...
            if X.nodes[o]['tc_min']:
                ...
                if window > 1:
                    RR_tc = np.convolve(RR_t, np.ones(window,)/window, mode='full')[:RR_t.size]   
                else:
                    RR_tc = RR_t
                I_I1month = I_idf(1./12., X.nodes[o]['tc_min'])
                sim_o_T[o] = np.array([T_idf(intensity, X.nodes[o]['tc_min'])\
                            if intensity > I_I1month else 0. for intensity in RR_tc]) 
...
def readOutfallLevels(outfall_level_filename, hours):
    ...
    t_res = int(fl.index[1][-8:-6])*60 + int(fl.index[1][-5:-3]) # data resolution temporal
...
def readTable(X, keyword, version):
    ...
    d = table.copy()
    d.loc[:,'d'] = d.loc[:,'chambvolume_m3'].values/d.loc[:,'chambarea_m2'].values
    d.loc[:,'NS'] = d.loc[:,['n_ROmq_m3ds', 'n_ROmro_m3ds']].max(axis = 1).values*300 / d.loc[:,'chambvolume_m3'].values
    d.loc[:,'PS'] = d.loc[:,['nc_Qmq_m3ds','nc_Qmo_m3ds']].max(axis = 1).values / d.loc[:,'Qmax_m3ds'].values
...
def mineFrequentSubgraph(graphs, minfreq, undirectional):
    ...
    if undirectional:
        ...
        ).mine_graphs(graphs,minimum_frequency=minfreq, find_paths_only = False, single_rooter= False, close_graph=True, store_embeddings=False)
    else:
        frequent_graphs = ParsemisMiner(
            "FrequentSubgraph", debug=True, mine_undirected=undirectional
        ).mine_graphs(graphs,minimum_frequency=minfreq, find_paths_only = False, single_rooter= False, close_graph=True, store_embeddings=True)
    # get nodes
...
def mineFrequentForest(X, f_datas, T, event_thres, minfreq):
    ...
    for c_nodes in forest_cluster_nodes:
        ...
        if event > event_thres: # only mine when at least 3 times flooded
            c_frequent_graphs, c_frequent_nodes = mineFrequentSubgraph(c_trees, minfreq, undirectional = 'True')
            print('\t', int(np.ceil(int(minfreq[:2])/100 * event)),'/', event, c_frequent_nodes)
...
def runGraphModel(X, rgRR, rg_id):
    ...
    # sim_n_RO = I(R,t)cont_area
    sim_n_RO = {n:linearReservior(sim_n_RR[n]/(10**3) * X.nodes[n]['cont_area_m2'], 15.) \
                            if n in sim_n_RR.keys() and X.nodes[n]['type'].lower() not in ['outfall'] \
                            else np.zeros(shape = (sim_n_RR_dur,)) for n in X.nodes}
    # sim_nc_RO = I(R,t)us_area 
    # sim_nc_Q = I(R,tc)us_area
    sim_nc_RO = {}
    sim_nc_Q = {}
    ...
    for n in XX.nodes:
        ns = list(nx.dfs_postorder_nodes(XX,n))
        # subcatchment runoff - accumulation in space (sum)
        ns_ = set(ns) - (set(ns) - set(sim_n_RO.keys())) # filter out nodes that have no inflow
    ...
    for o in outfalls:
        ns = list(nx.dfs_postorder_nodes(XX,o))
        # subcatchment rainfall rate - average in space (sum)
        ns_ = set(ns) - (set(ns) - set(sim_n_RR.keys())) # filter out nodes that have no inflow
        ...
        if not np.any(np.isnan(RR_t)):
            ...
            if X.nodes[o]['tc_min']:
                ...
                sim_o_T[o] = np.array([T_idf(intensity, X.nodes[o]['tc_min'])\
                            if intensity > I_I1month else 0. for intensity in RR_tc]) 
    sim_n_RO = pd.DataFrame.from_dict(sim_n_RO); sim_n_RO.index = rgRR.index
...
def getPredictors(X, sim_n_RO, sim_nc_RO, sim_nc_Q, sim_o_T):
    ...
    d.loc[:,'OT'] = d.loc[:,'o_Tmq_yr']; d.loc[:,'OT'] = d.loc[:,'OT'].round(1); d.loc[d.OT < 0.1, 'OT'] = 0.01
