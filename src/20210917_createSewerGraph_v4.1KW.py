...
conn_filename = sewer_dir + "Connections.shp" # Sewer connections shapefile needed for Delft3D use
...
for i in range(featureCount):
    kw.append(points_gpd['ManholeNam'][i]) # used to be .lower() (and with a different package but I digress)
...
for feature in layer:
    ...
    link_shape.append("Circle")#feature.GetField("shape_1")) < Should probably be expanded to use DefName
    capacity.append(None)#feature.GetField("capacity"))
    gradient.append((feature.GetField("Level sourc")-feature.GetField("Level targe"))/feature.GetField("Length")) # calculate gradient in same line
    ...
    sediment.append(None)#feature.GetField("sed_depth")/1000.0) #Dflow does nothing with this sediment bs
    roughness.append(3.0) #feature.GetField("bot_rcw")) # For some reason doesn't export and has to be in mm instead of m. Assume concrete value given in D-Flow, look at again later.
...
for feature in layer:
    ...
    structure_overview[name] = [type, upstream_id, downstream_id, feature_length, upstream_level, downstream_level]
...
for feature in layer:
    ...
    sub_slope.append(0.00006666666) #sub_slope.append(feature.GetField("cat_slope")) #NOTE: obvs not really 0
...
for k in nodes:
    ...
    a = (0 if k not in node_area or node_area[k] is None else node_area[k])  # EDITED: to m2 instead of ha
    ga = (0 if k not in node_grossarea or node_grossarea[k] is None else node_grossarea[k])  # ha to square meters
    ...
    X.add_node(k, type=t, chambfloor_m=cf, chambroof_m=cr, chambarea_m2=ca, chambvolume_m3=v, subcatslope= s,
               cont_area_m2=a, gross_area_m2=ga, geo=g, subcat_id=i)
