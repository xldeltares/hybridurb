...
def _predict(
    ...
    if isinstance(
        rgRR.index[0], str
    ):  # extra parsing of datetime #TODO improve when standard timeseries becomes available
        rgRR.index = pd.to_datetime(
            rgRR.index, format="%d/%m/%Y %H.%M", errors="ignore"
        )
    ...
    d = getPredictors_option1(
        X, sim_nc_RO, sim_nc_Q, sim_o_T
    )  # ['NS', 'PS', 'd',  'OT','Cat']
    d["n_F"] = d["d"] * 0
    for v in ["d", "NS", "PS", "OT"]:
    ...
    # mean prediction
    y_pred = model.predict(X_test)
    y_pred = y_pred.to_frame(name=ens_id)
...
class NowcastRunner(object):
    ...
    :param root_dir: Path object. Root directory containing model, input, and output subdirectories.
    :param t0: String. The start time for simulation, in the format 'yyyymmddHHMM', e.g. '201605300900'.
    :param name: String. A name for the runner instance, defaults to an empty string.
    :param crs: String. Coordinate reference system for the geographic data, defaults to 'EPSG:4326'.

    This method initializes various directory paths, the start time, and the name for the runner instance.
    It also calls the _initialise_model method to initialize the graph, lambda and statistical models.
    """

    def __init__(self, root_dir: Path, t0: str, name: str = "", crs="EPSG:4326"):
        """
        Loads the graph, lambda, and statistical models from the model directory.

        This method reads the graph model (a networkx graph), lambda model (for Box-Cox transformation),
        and statistical model (for logistic regression) from the respective pickled files in the model directory.
        It returns a tuple containing these three models.
        """
        self.t0 = t0
        self.name = name
        self.crs = crs
    ...
    def _initialise_rainfall(self):
        ...
        # rg_id id predefined
        at_time = self.t0
        rgrr_filenames = glob.glob(f"{self._input_dir}/{at_time}*ens*.csv")
        rgid_filenames = [
            Path(rgrr_filenames[0]).with_name("grid_to_node_mapping.csv")
            for i in range(len(rgrr_filenames))
        ]
        ...
        # read mapping
        rgID_list = [
            pd.read_csv(rgid_filename, index_col=0) for rgid_filename in rgid_filenames
        ]
        # get event list
        event_list = [
            re.findall("ens([0-9]+)", Path(rgrr_filename).name)[0]
            for rgrr_filename in rgrr_filenames
        ]
    ...
    def run(self):
        ...
        This method runs the simulation by reading rainfall data, running the graph model, preparing predictors,
        performing logistic regression, and writing the output to a CSV file in the output directory.
        It also prints the time taken for the simulation.
    ...
    def run_preadaptor(self):
        """
        Runs a pre-adaptor for FEWS.
        Converts rainfall inputs prepared by Delft-FEWS into rainfall inputs supported by HybridUrb.

        This method reads the rainfall data file prepared by Delft-FEWS, converts the unit from mm to mm/hour,
        and writes the converted data to a CSV file in the input directory.
        ...
        dt_s = (
            (da.time.values[1] - da.time.values[0])
        ...
        for realisation in da["realization"].values:
            ...
            di = (
                da["I_fc"]
                .sel(realization=realisation, drop=True)
                .drop_vars(["lat", "lon"])
            )
            # format dataframe
    ...
    def run_postadaptor(self):
        """
        Runs a post-adaptor for FEWS.
        Converts outputs generated by HybridUrb to standard netcdf files supported by Delft-FEWS.

        This method prepares the output data, writes it to a netCDF file in the output directory,
        and also writes a grid file containing the node locations to a netCDF file in the output directory.
        ...
        # prepare output grid
        node_loc = {
            n: i for i, n in enumerate(y_preds.index)
        }  # FIXME for now only non-nan locations are written
        ...
        ds = grid.to_dataset()
        ds.mesh1d_node_x.attrs = {
            "units": "m",
            "standard_name": "projection_x_coordinate",
            "long_name": "x-coordinate of mesh nodes",
        }
        ds.mesh1d_node_y.attrs = {
            "units": "m",
        ...
        # time dimension
        ds.coords["analysis_time"] = xr.DataArray(
            [analysis_time], dims=["analysis_time"]
        )
        ds.analysis_time.attrs = {
            "standard_name": "forecast_reference_time",
            "long_name": "forecast_reference_time",
        }
        ds.coords["time"] = xr.DataArray(
            pd.date_range(start=analysis_time, freq="5T", periods=2), dims=["time"]
        )
        ...
        ds.realization.attrs = {
            "standard_name": "realization",
            "long_name": "Index of an ensemble member within an ensemble",
            "units": "1",
        }
        ds["mesh1d_probability"] = xr.DataArray(
            np.stack([probability.T, probability.T]),
            dims=["time", "realization", grid.node_dimension],
        )
        ds.mesh1d_probability.attrs = {
            "mesh": "mesh1d",
            "location": "node",
            "standard_name": "thunderstorm_probability",
            "long_name": "Probability of flooding",
            "units": "1",
            "grid_mapping": "",
        }
        ds.to_netcdf(
            self._output_dir.joinpath(f"{self.t0}_predictions_at_nodes_ensemble.nc")
        )

        # do not use realization, run in loop. (NOTE deprecate this when the above becomes supported).
        for r in realization:
            ds["mesh1d_probability"] = xr.DataArray(
                np.stack([probability[:, r], probability[:, r]]),
                dims=["time", grid.node_dimension],
            )
            ds.mesh1d_probability.attrs = {
                "mesh": "mesh1d",
                "location": "node",
            ...
            ds.to_netcdf(
                self._output_dir.joinpath(f"{self.t0}_predictions_at_nodes_{r}.nc")
            )
    ...
    def run_fews(self):
        ...
        This method calls the run_preadaptor, run, and run_postadaptor methods in sequence to perform
        the entire FEWS process. It prints messages before and after each of these steps.
