...
def export_gdf(G: nx.Graph, out_dir: Path):
    ...
    _nodes[["node_name", "nodeID", "geometry"]].to_file(
        Path(out_dir).joinpath("network_nodes.shp")
    )
    return _nodes
...
def drawHazardMap(X, y_pred, ft=None):
    ...
    if ft is None:
        y_pred_multi = pd.cut(
            y_pred.values, [-1, 0.25, 0.5, 0.75, 1], labels=[0, 0.25, 0.5, 0.75]
        ).astype(
            float
        )  # categorized
        y_pred_multi = pd.DataFrame(
            data=y_pred_multi[..., np.newaxis], index=y_pred.index
        )
        ...
        nx.draw_networkx_nodes(
            y_pred_multi[y_pred_multi > 0].index,
            pos,
        ...
        y_pred_multi = pd.cut(
            ...
            float
        )  # categorized #FIXME new: the mean is fixed for y_pred, meaning that confidence intercL IA NOR AUPPOER
        y_pred_multi = pd.DataFrame(
            data=y_pred_multi[..., np.newaxis], index=y_pred.index
        )
        y_pred_multi = y_pred_multi[y_pred_multi > 0]
        ft_pred_multi = pd.cut(
            ft.values, [-1, 30, 60, 90, 120], labels=[0.5, 1, 1.5, 2]
        ).astype(float)
        ft_pred_multi = pd.DataFrame(
            data=ft_pred_multi[..., np.newaxis], index=ft.index
        )
        ...
        nx.draw_networkx_nodes(
            X,
            pos,
            nodelist=list(ft_pred_multi.index),
            node_color=ft_pred_multi,
            cmap="jet_r",
            vmin=0,
            vmax=2,
            node_size=10,
